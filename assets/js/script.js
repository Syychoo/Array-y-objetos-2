...
// Ejercicio 4
let deparments = [radiologia, traumatologia, dental]

document.write('<h2>Pacientes de la clínica:</h2>')
deparments.forEach(d=>{
    d.forEach(data=>{
        document.write(`<p>${data.paciente}</p>`)
    })
})

// Ejercicio 5

let dentalIsapre = dental.filter(data=>data.prevision=='ISAPRE')

document.write('<h2>ISAPRE en Dental:</h2>')
dentalIsapre.forEach(data=>{
    document.write(`<p>${data.paciente} - ${data.prevision}</p>`)
})

// Ejercicio 6

let traumatologiaFonasa = traumatologia.filter(data=>data.prevision=='FONASA')

document.write('<h2>FONASA en Traumatología:</h2>')
traumatologiaFonasa.forEach(data=>{
    document.write(`<p>${data.paciente} - ${data.prevision}</p>`)
})
